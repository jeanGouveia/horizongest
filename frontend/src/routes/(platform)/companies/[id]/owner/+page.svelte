<script lang="ts">
	...
	onMount(async () => {
		...
		companyID = parseInt(pathParts[4]);
		await loadOwner();
	});
	...
	async function loadOwner() {
		loading = true;
		try {
			const response = await fetch(`/api/platform/companies/${companyID}/owner`);
			if (response.ok) {
	...
	async function resetPassword() {
		...
		try {
			const response = await fetch(`/api/platform/companies/${companyID}/owner/reset-password`, {
				method: 'POST',
				headers: {
	...
	async function toggleUserBlock() {
		...
		try {
			const response = await fetch(`/api/platform/users/${owner.id}/${action}`, {
				method: 'POST'
			});
...
<div class="owner-container">
	<header class="owner-header">
		<div class="header-content">
			<Button variant="secondary" on:click={() => goto(`/platform/companies/${companyID}`)}>← Voltar</Button>
			<div>
		...
		<div class="owner-content">
			<Card class="owner-card">
				...
				<div class="detail-row">
					...
					<span>{new Date(owner.created_at).toLocaleString('pt-BR')}</span>
				</div>

				<div class="owner-actions">
					<Button variant="secondary" on:click={() => showResetPasswordModal = true}>
						Redefinir Senha
					</Button>
					<Button
						variant={owner.active ? 'secondary' : 'primary'}
						on:click={toggleUserBlock}
					>
		...
		<div class="modal-overlay" on:click={() => showResetPasswordModal = false}>
			<Card class="modal-card">
				...
				<div class="modal-actions">
					<Button variant="secondary" on:click={() => showResetPasswordModal = false}>Cancelar</Button>
					<Button variant="primary" on:click={resetPassword}>Redefinir</Button>
				</div>
			</Card>
		</div>
	{/if}
